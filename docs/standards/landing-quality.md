...
3. **Visual direction and scenes** — antes del master se define qué significa excelencia premium en este proyecto y qué baseline actual debe superar. El master debe demostrar ese umbral y la landing conservarlo. Cada sección contrasta su baseline con una oportunidad de producción de alto valor, separa ganancia real, equivalente simple y ruido caro, y adopta la forma mínima que preserve el valor. Las foundations se extraen de esas decisiones. El hero supera pruebas de integración, eliminación e intercambiabilidad; el cuerpo conserva el lenguaje sin repetir una receta. Gradientes azul/morado, sans geométrica, hero centrado, pill CTA, tres icon cards, sombras difusas, blobs, grain o glows son sospechosos cuando aparecen por defecto, pero nunca prohibiciones independientes del contexto.
