...
2. Define at least two content/landing flows, then run the structural interchangeability challenge before selection. Reject the default category sequence when sections could be exchanged without changing meaning.
...
7. Audit those renders first as one scroll narrative and then by scene. Map anchor/proof/support/atmosphere/transition/rest/climax/closure, derive the minimum sufficient asset set and remove duplicate jobs. Decide format separately from behavior: background/lateral/full-bleed/object/texture/sequence/video/real-3D/none versus static/hover/sticky/parallax/pinned/playback/interactive-3D. Require a narrative trigger, real layer decomposition and mobile/reduced fallback where applicable. Then decide `IMAGE | NO_IMAGE` and write `IH-*` briefs; do not generate final landing imagery inside this pipeline.
